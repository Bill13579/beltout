...
def run_conversion(state, mode, progress=gr.Progress(track_tqdm=True)):
    ...
    def process_chunk(wav_chunk_s3, wav_chunk_pitchmvmt):
        with torch.inference_mode():
            ...
            waveform_16k_tensor_s3 = torchaudio.transforms.Resample(model.sr, S3_SR).to(device)(waveform_24k_tensor_s3)
            waveform_24k_tensor_pitchmvmt = torch.from_numpy(wav_chunk_pitchmvmt).float().to(device).unsqueeze(0)
            waveform_16k_tensor_pitchmvmt = torchaudio.transforms.Resample(model.sr, S3_SR).to(device)(waveform_24k_tensor_pitchmvmt)
            
            # Get S3 tokens and speaker embedding
            s3_tokens, _ = model.s3gen.tokenizer(waveform_16k_tensor_s3)
